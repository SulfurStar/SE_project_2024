...
const fetchVisitRecord = async () => {
  try {
    const response = await fetch(`/api/visitation/get-visit-record/${userId}`)
    const result = await response.json()
    if (result.success) {
      formData.value = { ...formData.value, ...parseFormString(result.data.info_teacher) }
    } else {
      error.value = '無法獲取訪視紀錄'
    }
  } catch (err) {
    error.value = '請求失敗'
  } finally {
...
const parseFormString = (formString) => {
  const data = {};
  formString.split('\n').forEach(line => {
